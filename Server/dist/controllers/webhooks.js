...
export const stripeWebhooks = async (request, response) => {
    ...
    let event;
    try {
        event = Stripe.webhooks.constructEvent(request.body, signature, process.env.STRIPE_WEBHOOK_SECRETE);
    }
    catch (err) {
    ...
    switch (event.type) {
        case 'payment_intent.succeeded': {
            ...
            if (!userData || !courseData)
                break;
            courseData.enrolledStudents.push(userData._id); // Assuming we push ID string as per model ref: 'User'. But User model _id is String.
